...
{ 
	...
	void Create()
	{
		...
		LobbyGUI[0] = graphics.CreateText(NULL, 8, "&col[ffffff]等待玩家加入", 0.5, 0.15, true);
		LobbyGUI[1] = graphics.CreateText(NULL, 8, "还需 &col[ff0000]0 &col[ffffff]名玩家以开始", 0.5, 0.2, true);
	...
	void Update()
	{
		if(!Round::IsStarted()) {
			...
			if(c > 0) {
				LobbyGUI[1].SetText("还需 &col[ff0000]" + c + " &col[ffffff]名玩家以开始");
			}
			else {
				SetTimer(GetTimer() - 1);
				LobbyGUI[1].SetText("距离开始还有 &colr[10 200 100]" + ConvertIntToTime(GetTimer(), false));
				if(GetTimer() <= 0) 
				{
					// Starting round
					Round::Start();
